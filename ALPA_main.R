...
#
#
ALPA <- function(FileDEM, FileLandslides, MinGrpPntCnt = 3, MinGrpAcrDst = 0, MinEndAptRto = 0, MaxEndDvtAgl = 180, MinStpHrzLen = 30, EndHdlAgrCfg = 1, DEMSliding = 0, FilePrefix = "", OutputTemp = F) {
  #
  # check input
  ...
  for (i in 1:count_polygons) {
    ...
    spldf_profile <- f_lasld_split(raster_dem, pPolygons, MinGrpPntCnt, MinGrpAcrDst, MinEndAptRto, MaxEndDvtAgl, MinStpHrzLen, EndHdlAgrCfg, DEMSliding, FilePrefix, i, OutputTemp)
    #
    # update list
...
#
#
f_lasld_split <- function(pRasterDEM, pPolygons, MinGrpPntCnt, MinGrpAcrDst, MinEndAptRto, MaxEndDvtAgl, MinStpHrzLen, EndHdlAgrCfg, DEMSliding, FilePrefix, FileID, OutputTemp) {
  # #
  # # show step
  ...
  while (TRUE) {
    ...
    if (OutputTemp || Finalization) {
      ...
      #
      # get areas
      list_areas <- f_strips_areas(pRasterDEM, DEMSliding, list_strip_sp, strip_nodes_for_sp)
      #
      # get 
    ...
    if (OutputTemp) {
      ...
      spldf_profile <- f_pnts_save_proflle(strip_nodes, paras_strip, c(MinGrpPntCnt, MinGrpAcrDst, MinEndAptRto, MaxEndDvtAgl, MinStpHrzLen, EndHdlAgrCfg, DEMSliding), raster_CRS, file_out, FileID)
      #
      # save paras
...
#
#
f_strips_areas <- function(pRasterDEM, DEMSliding, list_strip_sp, strip_nodes) {
  #
  # get count
  ...
  #
  # if input DEM represents the sliding surface or not
  if (DEMSliding == 0) {
    #
    # get, use s
...
f_pnts_save_proflle <- function(pnts_prfl, paras_strip, paras_input, CRS_out, file_out, LineID = 1) {
  ...
  #
  # get data.frame
  paras_input <- data.frame(iMGPC = paras_input[1], iMGAD = paras_input[2], iMEAR = paras_input[3], iMEDA = paras_input[4], iMSHL = paras_input[5], iEHAC = paras_input[6], iDEMS = paras_input[7])
  #
