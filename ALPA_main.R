...
#
#
ALPA <- function(FileDEM, FileLandslides, MinGrpPntCnt = 3, MinGrpAcrDst = 0, MinEndAptRto = 0, MaxEndDvtAgl = 180, MinStpHrzLen = 30, EndHdlAgrCfg = 3, FilePrefix = "", OutputTemp = F) {
  #
  # check input
  ...
  for (i in 1:count_polygons) {
    ...
    spldf_profile <- f_lasld_split(raster_dem, pPolygons, MinGrpPntCnt, MinGrpAcrDst, MinEndAptRto, MaxEndDvtAgl, MinStpHrzLen, EndHdlAgrCfg, FilePrefix, i, OutputTemp)
    #
    # update list
...
#
#
f_lasld_split <- function(pRasterDEM, pPolygons, MinGrpPntCnt, MinGrpAcrDst, MinEndAptRto, MaxEndDvtAgl, MinStpHrzLen, EndHdlAgrCfg, FilePrefix, FileID, OutputTemp) {
  # #
  # # show step
  ...
  while (TRUE) {
    ...
    if (OutputTemp) {
      ...
      spldf_profile <- f_pnts_save_proflle(strip_nodes, paras_strip, c(MinGrpPntCnt, MinGrpAcrDst, MinEndAptRto, MaxEndDvtAgl, MinStpHrzLen, EndHdlAgrCfg), raster_CRS, file_out, FileID)
      #
      # save paras
    ...
    for (i in 1:length(list_pnts)) {
      ...
      if (grpm_i == 0) {
        ...
        if (bool_initial) {
          ...
          #
          # check
          if (is.na(list_end_pnts_i_upper)) {
            #
            # set grpm
            grpm <- 1
            #
        ...
        if (bool_distal) {
          ...
          #
          # check
          if (is.na(list_end_pnts_i_lower)) {
            #
            # set grpm
            grpm <- 1
            #
...
f_pnts_save_proflle <- function(pnts_prfl, paras_strip, paras_input, CRS_out, file_out, LineID = 1) {
  ...
  #
  # get data.frame
  paras_input <- data.frame(iMGPC = paras_input[1], iMGAD = paras_input[2], iMEAR = paras_input[3], iMEDA = paras_input[4], iMSHL = paras_input[5], iEHAC = paras_input[6])
  #
