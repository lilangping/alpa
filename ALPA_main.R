...
#
#
ALPA <- function(FileDEM, FileLandslides, MinGrpPntCnt = 3, MinGrpAcrDst = 0, MinEndAptRto = 0, MaxEndDvtAgl = 180, MinStpHrzLen = 30, MinStpCnt = 2, FilePrefix = "", OutputTemp = F) {
  #
  # check input
  ...
  for (i in 1:count_polygons) {
    ...
    spldf_profile <- f_lasld_split(raster_dem, pPolygons, MinGrpPntCnt, MinGrpAcrDst, MinEndAptRto, MaxEndDvtAgl, MinStpHrzLen, MinStpCnt, FilePrefix, i, OutputTemp)
    #
    # update list
...
#
#
f_lasld_split <- function(pRasterDEM, pPolygons, MinGrpPntCnt, MinGrpAcrDst, MinEndAptRto, MaxEndDvtAgl, MinStpHrzLen, MinStpCnt, FilePrefix, FileID, OutputTemp) {
  # #
  # # show step
  ...
  while (TRUE) {
    ...
    if (OutputTemp || (min(data.frame(list_grpm)) == 1)) {
      ...
      #
      # check, at least two (or MinStpCnt) strip, do not allow no station
      if (count_strip < MinStpCnt) {
        #
        # update
        count_strip <- max(MinStpCnt, 2)
      }
      #
      # strip might be merged
      while (T) {
        #
        # get strips
        ...
        #
        # if MinStpCnt or not
        if (length(list_strips[[1]]) >= MinStpCnt) { break }
        else { count_strip <- count_strip + 1 }
      }
      #
      # get
    ...
    if (OutputTemp) {
      ...
      #
      # save strips
      file_out <- paste(FilePrefix, "_l", sprintf("%05d", FileID), "_pnts_grps", sprintf("%03d", pnts_split[nrow(pnts_split), "grp"]), "_strips_triangles.xlsx", sep = "", collapse = NULL)
      f_pnts_save_xls(strip_triangle_df, file_out)
      #
      # save shp
      file_out <- paste(FilePrefix, "_l", sprintf("%05d", FileID), "_pnts_grps", sprintf("%03d", pnts_split[nrow(pnts_split), "grp"]), "_strips_triangles", sep = "", collapse = NULL)
      ...
      spldf_profile <- f_pnts_save_proflle(strip_nodes, paras_strip, c(MinGrpPntCnt, MinGrpAcrDst, MinEndAptRto, MaxEndDvtAgl, MinStpHrzLen, MinStpCnt), raster_CRS, file_out, FileID)
      #
      # save paras
    ...
    if (min(data.frame(list_grpm)) == 1) {
      ...
      #
      # save strips
      file_out <- paste(FilePrefix, "_l", sprintf("%05d", FileID), "_pnts_grps_strips_triangles.xlsx", sep = "", collapse = NULL)
      f_pnts_save_xls(strip_triangle_df, file_out)
      #
      # save shp
      file_out <- paste(FilePrefix, "_l", sprintf("%05d", FileID), "_pnts_grps_strips_triangles", sep = "", collapse = NULL)
      f_save_list2sp(strip_triangle_sp, strip_triangle_df, raster_CRS, file_out)
      #
      # save nodes
...
#
#
f_pnts_path_lines <- function(list_bnd_sides_right, list_bnd_sides_left, column_anchors, count_strip) {
  # #
  # # remove Cz
...
f_strips <- function(pPolygons, list_path, count_strip) {
  ...
  #
  # get path lines
  list_path_lines <- f_pnts_path_lines(list_bnd_sides_right, list_bnd_sides_left, column_anchors, count_strip)
  #
  # get 
  ...
  for (i in 1:count_cl) {
    ...
    #
    # get index
    idx_station_in_nodes <- which(strip_nodes_for_sp[, "station"] == i)
    #
    # check
    if (bool_ints == TRUE) {
      #
...
f_strips_areas <- function(pRasterDEM, list_strip_sp, strip_nodes) {
  ...
  for (i in 1:nrow(column_ints_rmdp)) {
    ...
    #
    # get Cx and Cy
    Cx <- column_ints_rmdp[i, "Cx"]
    Cy <- column_ints_rmdp[i, "Cy"]
    #
    # get dists
    ...
    for (k in 1:length(column_dists_nebr)) {
      #
      # append
      column_dists_diff <- rbind(column_dists_diff, abs(column_dists_upper[k] + column_dists_lower[k] - column_dists_nebr[k]))
    }
    #
    # get position
    column_positions <- rbind(column_positions, which(column_dists_diff == min(column_dists_diff)))
  }
  #
  ...
  # anway, you need to search one by one
  ...
  for (i in 1:count_node) {
    ...
    for (k in 1:nrow(column_triangles)) {
      ...
      if (pip != 0) {
        ...
        Czs <- -(nx*(node[1, "Cx"]-Cx1) + ny*(node[1, "Cy"]-Cy1)) / nz + Cz1
        #
        # update
  ...
  for (i in 2:count_node) {
    ...
    if (strip_nodes2[i, "type"] == "Station: Strip boundary") {
      ...
      column_Lstps <- rbind(column_Lstps, strip_nodes2[i, "Lalls"])
    }
  }
  #
  # append
  column_Lhrzs <- rbind(column_Lhrzs, strip_nodes2[count_node, "Lhrz"])
  column_Lstps <- rbind(column_Lstps, strip_nodes2[count_node, "Lalls"])
  #
  # get length strip
  ...
  #
  # switch
  strip_nodes2 <- strip_nodes2[, c("Cx", "Cy", "Czd", "Czp", "Czs", "type", "grp", "station", "Lhrz", "Lalld", "Lallp", "Lalls")]
  #
  # get
  strip_Ahrz <- column_Ahrzs
  strip_Aall <- column_Astps
  # get
  strip_Lhrz <- column_Lhrzs
  strip_Lall <- column_Lstps
  # get
  strip_drops <- column_drops
  # get
  triangle_df <- column_triangles_df
  #
  # get sp attribute
  strip_df <- cbind(strip_Lhrz, strip_Lall, strip_Ahrz, strip_Aall, strip_drops)
  #
  # colnames
...
f_pnts_save_proflle <- function(pnts_prfl, paras_strip, paras_input, CRS_out, file_out, LineID = 1) {
  ...
  #
  # get data.frame
  paras_input <- data.frame(iMGPC = paras_input[1], iMGAD = paras_input[2], iMEAR = paras_input[3], iMEDA = paras_input[4], iMSHL = paras_input[5], iMSC = paras_input[6])
  #
