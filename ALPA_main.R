...
#
#
ALPA <- function(FileDEM, FileLandslides, MinGrpPntCnt = 3, MinGrpAncDist = 0, FilePrefix = "", OutputTemp = F) {
  #
  # check input
  library(dendextend)
  if (is.natural.number(MinGrpPntCnt) == F) {
    #
    # disp
    disp("Error: MinGrpPntCnt is not a natural number.")
    #
    # return
    return(NA)
  }
  ...
  if (MinGrpAncDist < 0) {
    #
    # disp
    disp("Error: MinGrpAncDist is negative.")
    #
    # return
    return(NA)
  }
  ...
  #
  # get file
  file_out_profile <- paste(FilePrefix, "_lasd_profile2d", sep = "", collapse = NULL)
  file_out_xlsx <- paste(FilePrefix, "_lasd_profile2d.xlsx", sep = "", collapse = NULL)
  #
  # split
  for (i in 1:count_polygons) {
    ...
    spldf_profile <- f_lasld_split(FileDEM, pPolygons, MinGrpPntCnt, MinGrpAncDist, FilePrefix, i, OutputTemp)
    #
    # update list
    ...
    #
    # show progress
    disp( paste("Process the ", as.character(i), "/", as.character(count_polygons), " landslide done.", sep = "") )
  }
}
#
#
#
f_lasld_split <- function(FileDEM, pPolygons, MinGrpPntCnt, MinGrpAncDist, FilePrefix, FileID, OutputTemp) {
  #
  # library
  library(raster)
  library(rgdal)
  ...
  while (TRUE) {
    ...
    if (OutputTemp) {
      ...
      #
      # save paras
      file_out <- paste(FilePrefix, as.character(FileID), "_pnts_grp", as.character(pnts_split[nrow(pnts_split), "grp"]), "_anchors_pln2d.xlsx", sep = "", collapse = NULL)
    ...
    if (min(data.frame(list_grpm)) == 1) {
      ...
      #
      # save paras
      file_out <- paste(FilePrefix, as.character(FileID), "_pnts_grp_anchors_pln2d.xlsx", sep = "", collapse = NULL)
      f_pnts_save_xls(spldf_profile@data, file_out)
      #
      # break
      break
    ...
    for (i in 1:length(list_pnts)) {
      ...
      if (grpm_i == 0) {
        ...
        if (pnts_split_distances_min <= MinGrpAncDist) {
          #
          # set grpm
          grpm <- 1
          #
...
f_pnts_anchors <- function(list_pnts, IDB_max) {
  ...
  if (count_pnts == 1) {
    ...
    #
    # get anchors, for initial and distal
    list_anchors <- f_pnts_anchor_end(pnts, c(lPntX, lPntY))
    anchor_end <- list_anchors[[1]]
    #
    # initial
    anchor_initial <- c()
    anchor_distal <- c()
    ...
    if (nrow(anchor_end) == 2) {
      #
      # split initial and distal
      if (anchor_end[1, 3] >= cPz) {
        #
        # use the first one
        anchor_initial <- anchor_end[1, ]
        anchor_distal <- anchor_end[2, ]
      }
      else {
        #
        # use the first one
        anchor_initial <- anchor_end[2, ]
        anchor_distal <- anchor_end[1, ]
      }
    }
    else if (nrow(anchor_end) == 1) {
      #
      # it is possible that only return one anchor_end
      # disp("Warning: The initial group of landslide elevation points does not have initial and distal anchors.")
      #
      # split initial and distal
      if (anchor_end[1, 3] >= cPz) {
        #
      ...
      #
      # according to Cz
      anchor_initial <- anchor_end[which.max(anchor_end[, "Cz"]), ]
      anchor_distal <- anchor_end[which.min(anchor_end[, "Cz"]), ]
    }
    #
    # get column_anchors
    column_anchors <- rbind(anchor_initial, centerP, anchor_distal)
  ...
  if (is.na(anchor_end_initial_edge)) {
    #
    # return
    return(NA)
  }
  ...
  if (nrow(anchor_end_initial_edge) >= 2) {
    # #
    # # return
    # return(NA)
    #
    # use the one with the maximum distance from anchor
    anchor_end_initial_edge <- list_anchors[[2]]
  }
  #
  ...
  anchor_end_initial_center <- cbind(list_anchors[[3]], "Initial group center", 1)
  ...
  #
  # is possible that f_pnts_anchor_end return NA?
  if (is.na(anchor_end_distal_edge)) {
    #
    # return
    return(NA)
  }
  #
  # possible return more than one?
  if (nrow(anchor_end_distal_edge) >= 2) {
    # #
    # # return
    # return(NA)
    #
    # use the one with the maximum distance from anchor
    anchor_end_distal_edge <- list_anchors[[2]]
  }
  #
  ...
  anchor_end_distal_center <- cbind(list_anchors[[3]], "Distal group center", (list_pnts[[count_pnts]])[1, "grp"])
...
f_pnts_anchor_end <- function(pnts, lPnt) {
  ...
  if (is.null(ints)) {
    #
    # # debugging
    # f_pnts_save_shp(pnts, CRS("+init=epsg:32643"), "pnts")
    #
    # disp
    disp("Warning: no intscts gotten, in defining anchors in an end group.")
    #
    # # return
    # return(NA)
    #
    # use the bnd point in the middle of all the bnd points
    # do not consider the situation where more than one intersect is expected
    #
    # odd or even
    if (count_bnd%%2 == 1) {
      #
      ...
      #
      # get ints
      intsX = (pnts_curve_bnd[idx1, "Cx"] + pnts_curve_bnd[idx2, "Cx"]) / 2
      intsY = (pnts_curve_bnd[idx1, "Cy"] + pnts_curve_bnd[idx2, "Cy"]) / 2
      # 
      # get ints
      ints = data.frame(intsX, intsY)
    }
    #
    # mark
    # right now, do not mark
    # it is somehow complicating
  }
  #
  ...
  if (is.na(count_intsct) || is.nan(count_intsct) || is.null(count_intsct) || isempty(count_intsct)) {
    #
    # disp
    disp("Error: empty intscts gotten, in defining anchors in an end group.")
    #
    # return
    return(NA)
  }
...
f_intersection_curves <- function(pnts1, pnts2) {
  ...
  y1 <- pnts1[, 2]
  #
  # must add the first point to the end
  # or, the circel will not close
  # which possibly induce missing of intersection
  x1 <- c(x1, pnts1[1, 1])
  y1 <- c(y1, pnts1[1, 2])
  #
  # get x and y
...
f_pnts_save_shp_pln <- function(pnts, CRS_out, file_out, LineID = 1) {
  ...
  #
  # get parameters from landslide profile
  paras <- f_profile2paras(pnts)
  #
  # get data_df
  data_df <- cbind(data.frame(ID = LineID), paras)
  #
  # get split line
...
f_profile2paras <- function(pnts) {
  ...
  #
  # get tortuosity vertical
  d_vertical <- sqrt((l_profile)^2 + 
                     (h_profile)^2)
  T_vertical <- s_profile / d_vertical
  #
  # get paras
  paras <- data.frame(Htot = -h_profile, Ltot = l_profile, miuapp = miuapp_profile,
                      Hmax = -min(h_series), Hint = H_integral,
                      Gmax = max(g_series), Gint = G_integral,
                      Tall = T_overall, Thor = T_horizontal, Tver = T_vertical)
  #
  # return
  return(paras)
}
...
f_data2spldf <- function(data_spl, data_df) {
  ...
  # get spl
  pLine <- Line(data_spl)
  pLines <- Lines(list(pLine), ID = "a")
  pSpatialLines <- SpatialLines(list(pLines))
  #
  # get df
  rownames(data_df) <- c("a")
  #
  # get
  spldf <- SpatialLinesDataFrame(pSpatialLines, data = data_df)
