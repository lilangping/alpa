...
ALPA <- function(InDEM, InLandslides, InParameters = 0, InCfgEndAnchorsInt = 1, InCfgEndAnchorsDit = 1, InCfgEndStripsInt = 1, InCfgEndStripsDit = 1, InOutputHierarchical = F) {
  ...
  #
  # import vector layer
  shp_lasld <- rgdal::readOGR(InLandslides)
  #
  # get polygons
  ...
  if (InParameters != 0) {
    #
    # read xlsx
    paras <- read.xlsx(InParameters, 1, header = TRUE)
  }
  #
  # get EndAnchorsInt
  if (is.character(InCfgEndAnchorsInt)) {
    #
    # read shapefile
    shp_EndAnchorsInt <- rgdal::readOGR(InCfgEndAnchorsInt)
    #
    # get coords
  ...
  if (is.character(InCfgEndAnchorsDit)) {
    #
    # read shapefile
    shp_EndAnchorsDit <- rgdal::readOGR(InCfgEndAnchorsDit)
    #
    # get coords
  ...
  if (is.character(InCfgEndStripsInt)) {
    #
    # read shapefile
    shp_EndStripsInt <- rgdal::readOGR(InCfgEndStripsInt)
    #
    # get lines
  ...
  if (is.character(InCfgEndStripsDit)) {
    #
    # read shapefile
    shp_EndStripsDit <- rgdal::readOGR(InCfgEndStripsDit)
    #
    # get lines
...
f_pnts_clustering <- function(pnts, raster_cellsize, raster_CRS) {
  ...
  for (i in 1:length(pnts_polygon_sf_sparts_sp@polygons)) {
    ...
    for (j in 1:length(pnts_polygon_sf_sparts_sp@polygons[[i]]@Polygons)) {
      ...
      if (is.na(crs(pnts_polygon_sf_sparts_sp_pgn)))
...
f_pnts_path <- function(list_pnts, IDB_max, pPolygons, EndAnchorsInt, EndAnchorsDit, EndStripsInt, EndStripsDit, BoolDirection) {
  ...
  if (count_pnts == 1) {
    ...
    for (i in 1:nrow(pnts_bnd_ascending)) {
      ...
      lz2 <- 0
      #
      cp1 <- cross(c(lx_path, ly_path, lz_path), c(lx1, ly1, lz1))
      cp2 <- cross(c(lx_path, ly_path, lz_path), c(lx2, ly2, lz2))
      #
      if (cp1[3] > 0 && cp2[3] < 0) {
        #
...
f_pnts_path_lines <- function(list_bnd_sides_right, list_bnd_sides_left, column_anchors, CfgEndStripsInitial, CfgEndStripsDistal, MinStpHrzLen) {
  ...
  if (length(CfgEndStripsDistal) == 5) {
    ...
    pnts_curve_path_cut_distal <- list_cut_polyline_segment[[2]]
    #
    # get cut length
    pnts_curve_path_cut_distal_length <- sum(f_pnts_distances(pnts_curve_path_cut_distal[, c("Cx", "Cy")]))
    #
...
f_strips <- function(pPolygons, list_path, CfgEndStripsInitial, CfgEndStripsDistal, MinStpHrzLen) {
  ...
  if (F) {
    ...
    if (length(CfgEndStripsInitial) == 5) {
      #
      CfgEndStripsInitial <- data.frame(rbind(CfgEndStripsInitial[1:2], CfgEndStripsInitial[3:4]))
      colnames(CfgEndStripsInitial) <- c("Cx", "Cy")
      f_pnts_save_points(CfgEndStripsInitial, NA, "CfgEndStripsInitial")
    }
    #
    #save
    if (length(CfgEndStripsDistal) == 5) {
      #
      CfgEndStripsDistal <- data.frame(rbind(CfgEndStripsDistal[1:2], CfgEndStripsDistal[3:4]))
      colnames(CfgEndStripsDistal) <- c("Cx", "Cy")
      f_pnts_save_points(CfgEndStripsDistal, NA, "CfgEndStripsDistal")
    }
  }
  #
  ...
  for (i in 1:count_cl) {
    ...
    # This throws an error, because xxx and xxx
    # do not have exactly the same coordinate reference system (CRS)
    sp::proj4string(sshp) <- crs(pSpatialPolygons)
    #
  ...
  for (i in 1:count_cl) {
    ...
    # This throws an error, because xxx and xxx
    # do not have exactly the same coordinate reference system (CRS)
    sp::proj4string(pnt_upper_fwd_shp) <- crs(pSpatialPolygons)
    sp::proj4string(sfc_POLYGON1_sp) <- crs(pSpatialPolygons)
    sp::proj4string(sfc_POLYGON2_sp) <- crs(pSpatialPolygons)
    #
    # get over
...
f_strips_package <- function(list_pnts, IDB_max, pPolygons, EndAnchorsInt, EndAnchorsDit, EndStripsInt, EndStripsDit, MinStpHrzLen, pRasterDEM) {
  ...
  if (length(EndStripsInt) == 5) {
    ...
    split_polygon_segment_areas <- st_area(split_polygon_segment)
    pPolygons_areas <- st_area(st_as_sf(sp::SpatialPolygons(list(pPolygons))))
    # get cut
...
f_intersection_curves <- function(pnts1, pnts2) {
  ...
  #
  # debugging, sl2 is invalid
  if (F) {
    #
    if (!rgeos::gIsValid(sl2)) {
      #
      # pause
      message("sl2 is invalid")
      #
      # return
      return("invalid")
    }
    ...
    out <- tryCatch (
      ...
      warning = function(cond) {
        ...
        for (i in 1:length(cond)) {
          if (cond[i] == "sl2 is invalid") { return(NULL) }
        }
      },
      #
      # Handler when an error occurs:
  ...
  if (F) { int.coords <- unique(int.coords) }
  #
  # return
  return(int.coords)
}
...
f_cut_polyline_segment <- function(polyline, segment) {
  ...
  polyline_sf <- st_sf(
    id = 1, 
    geometry = st_sfc(st_linestring(polyline))
  )
  # sf
  segment_sf <- st_sf(
    id = 2, 
    geometry = st_sfc(st_linestring(segment))
  )
  # #
  # # set crs
  ...
  intersection_pts <- st_intersection(polyline_sf, segment_sf)
  #
  # if no intersections
  ...
  for (i in 1:(count_node-1)) {
    #
    # sf
    polyline_segment_sf <- st_sf(
      id = 3, 
      geometry = st_sfc(st_linestring(polyline[i:(i+1), ]))
    )
    #
    # st_intersection
    intersection_pts <- st_intersection(polyline_segment_sf, segment_sf)
    #
    # check
    if (nrow(intersection_pts) == 1) {
      #
      # get coords
      intersections_coords <- st_coordinates(intersection_pts)
      # get XY
...
f_split_polygon_segment <- function(pPolygons, segment, RatioEXTD = NA) {
  ...
  sf_poly <- st_as_sf(pSpatialPolygons)
  #
  ...
  sf_line <- st_linestring(segment)
  #
  # 将线段转换为 sfc 对象，并确保坐标系与 Polygon 一致
  sf_sfc_line <- st_sfc(sf_line, crs = st_crs(sf_poly))
  #
  # 执行 Split 操作
  ...
  split_polys <- st_collection_extract(split_collection, type = "POLYGON")
  #
  # 返回结果（数量和切割后的 sf 对象）
  return(split_polys)
}
