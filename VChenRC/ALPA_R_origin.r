...
#
#
alpa_read_ogr <- function(path) {
  sf_obj <- suppressWarnings(sf::st_read(path, quiet = TRUE, stringsAsFactors = FALSE))
  sf_obj <- suppressWarnings(sf::st_zm(sf_obj, drop = TRUE, what = "ZM"))
  methods::as(sf_obj, "Spatial")
}

alpa_delete_shapefile <- function(file_base) {
  exts <- c(".shp", ".shx", ".dbf", ".prj", ".cpg", ".qix", ".fix", ".sbn", ".sbx", ".shp.xml")
  invisible(lapply(exts, function(ext) {
    path <- paste(file_base, ext, sep = "", collapse = NULL)
    if (file.exists(path)) {
      unlink(path)
    }
  }))
...
alpa_write_ogr <- function(obj, dsn = ".", layer, overwrite = TRUE, overwrite_layer = overwrite, driver = "ESRI Shapefile") {
  file_base <- if (dsn == ".") layer else file.path(dsn, layer)
  if (isTRUE(overwrite) || isTRUE(overwrite_layer)) {
    alpa_delete_shapefile(file_base)
  }
  sf::st_write(sf::st_as_sf(obj), paste(file_base, ".shp", sep = "", collapse = NULL), quiet = TRUE, append = FALSE)
  return(obj)
}

alpa_write_gdal <- function(obj, filename) {
  raster_obj <- if (inherits(obj, "Raster")) obj else raster::raster(obj)
  raster::writeRaster(raster_obj, filename, overwrite = TRUE)
  return(raster_obj)
}

alpa_g_is_valid <- function(obj) {
  valid <- suppressWarnings(sf::st_is_valid(sf::st_as_sf(obj)))
  all(valid)
}

alpa_g_intersection <- function(obj1, obj2, byid = TRUE) {
  sf1 <- suppressWarnings(sf::st_as_sf(obj1))
  sf2 <- suppressWarnings(sf::st_as_sf(obj2))
  geom1 <- suppressWarnings(sf::st_make_valid(sf::st_geometry(sf1)))
  geom2 <- suppressWarnings(sf::st_make_valid(sf::st_geometry(sf2)))
  inter <- suppressWarnings(sf::st_intersection(geom1, geom2))
  if (length(inter) == 0) {
    return(NULL)
  }
  inter_sf <- sf::st_sf(geometry = inter)
  if (nrow(inter_sf) == 0) {
    return(NULL)
  }
  methods::as(inter_sf, "Spatial")
}

ALPA <- function(InDEM, InLandslides, InParameters = 0, InCfgEndAnchorsInt = 1, InCfgEndAnchorsDit = 1, InCfgEndStripsInt = 1, InCfgEndStripsDit = 1, InOutputHierarchical = F) {
  #
  # get raster
  raster_dem <- raster::raster(InDEM)
  # get CRS
  raster_CRS <- sp::proj4string(raster_dem)
  #
  # import vector layer
  shp_lasld <- alpa_read_ogr(InLandslides)
  #
  # get polygons
  ...
  if (InParameters != 0) {
    #
    # read xlsx
    paras <- xlsx::read.xlsx(InParameters, 1, header = TRUE)
  }
  #
  # get EndAnchorsInt
  if (is.character(InCfgEndAnchorsInt)) {
    #
    # read shapefile
    shp_EndAnchorsInt <- alpa_read_ogr(InCfgEndAnchorsInt)
    #
    # get coords
  ...
  #
  # get EndAnchorsDit
  if (is.character(InCfgEndAnchorsDit)) {
    #
    # read shapefile
    shp_EndAnchorsDit <- alpa_read_ogr(InCfgEndAnchorsDit)
    #
    # get coords
  ...
  #
  # get EndStripsInt
  if (is.character(InCfgEndStripsInt)) {
    #
    # read shapefile
    shp_EndStripsInt <- alpa_read_ogr(InCfgEndStripsInt)
    #
    # get lines
    ...
    for (i in 1:count_lines) {
      ...
      #
      # append
      coords_EndStripsInt <- rbind(coords_EndStripsInt, c(coords[1, 1], coords[1, 2], coords[2, 1], coords[2, 2]))
      #
      # get direction
  ...
  #
  # get EndStripsDit
  if (is.character(InCfgEndStripsDit)) {
    #
    # read shapefile
    shp_EndStripsDit <- alpa_read_ogr(InCfgEndStripsDit)
    #
    # get lines
    ...
    for (i in 1:count_lines) {
      ...
      #
      # append
      coords_EndStripsDit <- rbind(coords_EndStripsDit, c(coords[1, 1], coords[1, 2], coords[2, 1], coords[2, 2]))
      #
      # get direction
      derection <- atan2(coords[2, 2] - coords[1, 2], coords[2, 1] - coords[1, 1])
      #
  ...
  #
  # initial list_spldf_path
  list_spldf_path <- list()
  #
  # get file
  file_out_path <- paste(gsub(".shp", "", InLandslides), "_paths2d", sep = "", collapse = NULL)
  file_out_png <- paste(gsub(".shp", "", InLandslides), "_paths2d.png", sep = "", collapse = NULL)
  file_out_xlsx <- paste(gsub(".shp", "", InLandslides), "_paths2d.xlsx", sep = "", collapse = NULL)
  #
  # split
  for (i in 1:count_polygons) {
    ...
    if (InParameters == 0) {
      #
      # get Name
      iName <- paste("lsd", sprintf("%06d", i), sep = "", collapse = NULL)
    ...
    if (is.character(InCfgEndAnchorsInt)) {
      ...
      #
      # debugging
      if (InCfgEndAnchorsInt != 1 && InCfgEndAnchorsInt != 2 && InCfgEndAnchorsInt != 3) {
        #
        # message
        message("Error: wrong input of InCfgEndAnchorsInt.")
      }
      #
      # negative, to distinguish from direction values.
      pEndAnchorsInt <- -InCfgEndAnchorsInt
    }
    #
    # get EndAnchorsDit
    if (is.character(InCfgEndAnchorsDit)) {
      ...
      #
      # debugging
      if (InCfgEndAnchorsDit != 1 && InCfgEndAnchorsDit != 2 && InCfgEndAnchorsDit != 3) {
        #
        # message
        message("Error: wrong input of InCfgEndAnchorsDit.")
      }
      #
      # negative, to distinguish from direction values.
      pEndAnchorsDit <- -InCfgEndAnchorsDit
    }
    #
    # get EndStripsInt
    if (is.character(InCfgEndStripsInt)) {
      #
      pEndStripsInt <- c(coords_EndStripsInt[i, ], directions_EndStripsInt[i, ])
    }
    else {
      #
      # debugging
      if (InCfgEndStripsInt != 1 && InCfgEndStripsInt != 2 && InCfgEndStripsInt != 3) {
        #
        # message
        message("Error: wrong input of InCfgEndStripsInt.")
      }
      #
      # negative, to distinguish from direction values.
      pEndStripsInt <- -InCfgEndStripsInt
    }
    #
    # get EndStripsDit
    if (is.character(InCfgEndStripsDit)) {
      #
      pEndStripsDit <- c(coords_EndStripsDit[i, ], directions_EndStripsDit[i, ])
    }
    else {
      #
      # debugging
      if (InCfgEndStripsDit != 1 && InCfgEndStripsDit != 2 && InCfgEndStripsDit != 3) {
        #
        # message
        message("Error: wrong input of InCfgEndStripsDit.")
      }
      #
      # negative, to distinguish from direction values.
      pEndStripsDit <- -InCfgEndStripsDit
    }
    #
    # split using one polygons
    spldf_path <- f_lasld_split(raster_dem, pPolygons, iName, pMinGrpPntCnt, pMinGrpAcrDst, pMinStpHrzLen, pMinEndRtoInt, pMinEndRtoDit, pEndAnchorsInt, pEndAnchorsDit, pEndStripsInt, pEndStripsDit, InOutputHierarchical)
    #
    # update list
    if (is.null(spldf_path) == F) {
      #
      # append list
      list_spldf_path[[length(list_spldf_path) + 1]] <- spldf_path
      #
      # list2shp
      data_df <- f_save_list2spldf(list_spldf_path, raster_CRS, file_out_path)
      #
      # save xlsx
      f_pnts_save_xls(data_df, file_out_xlsx)
      #
      # save png
      f_save_paths_png(raster_dem, shp_lasld, list_spldf_path, file_out_png)
      #
      # show progress
...
#
#
f_lasld_split <- function(pRasterDEM, pPolygons, iName, MinGrpPntCnt, MinGrpAcrDst, MinStpHrzLen, MinEndRtoInt, MinEndRtoDit, EndAnchorsInt, EndAnchorsDit, EndStripsInt, EndStripsDit, OutputHrcl) {
  # #
  # # show step
  ...
  while (TRUE) {
    ...
    if (OutputHrcl) {
      #
      # get temporary Cfg
      if (length(EndStripsInt) == 5) { CfgEndStripsInt <- EndStripsInt[5] }
      else { CfgEndStripsInt <- EndStripsInt }
      if (length(EndStripsDit) == 5) { CfgEndStripsDit <- EndStripsDit[5] }
      else { CfgEndStripsDit <- EndStripsDit }
      #
      # get path (centers and bnd sides)
      list_pnts_path <- f_pnts_path(list_pnts, IDB_max, pPolygons, EndAnchorsInt, EndAnchorsDit, CfgEndStripsInt, CfgEndStripsDit, F)
      #
      # check
    ...
    if (Finalization) {
      ...
      # check, if have valid
        ...
        spldf_path <- f_pnts_save_path(strip_nodes, paras_strip, iName, MinGrpPntCnt, MinGrpAcrDst, MinStpHrzLen, MinEndRtoInt, MinEndRtoDit, EndAnchorsInt, EndAnchorsDit, EndStripsInt, EndStripsDit, raster_CRS, file_out)
        #
        # save paras
        file_out <- paste(FilePrefix, "_pnts_grps_strips_nodes_pln2d.xlsx", sep = "", collapse = NULL)
        f_pnts_save_xls(spldf_path@data, file_out)
        #
        # break
        break
      }
    ...
    for (i in seq_along(list_pnts)) {
      ...
      if (grpm_i == 0) {
        ...
        if (length(list_clusters_upper) >= 2 || length(list_clusters_lower) >= 2) {
          ...
          for (k in seq_along(list_clusters_upper)) {
            #
            # get pnts
          ...
          for (k in seq_along(list_clusters_lower)) {
            #
            # get count
          ...
          if (length(list_clusters_lower_for_upper) >= 1) {
            #
            # update
            for (k in seq_along(list_clusters_lower_for_upper)) {
              #
              # to upper
          ...
          if (length(list_clusters_upper_for_lower) >= 1) {
            #
            # update
            for (k in seq_along(list_clusters_upper_for_lower)) {
              #
              # to lower
        ...
        upper_plane_invalid <- !is.data.frame(pnts_i_upper) && length(pnts_i_upper) == 1 && is.na(pnts_i_upper)
        lower_plane_invalid <- !is.data.frame(pnts_i_lower) && length(pnts_i_lower) == 1 && is.na(pnts_i_lower)
        if (upper_plane_invalid || lower_plane_invalid) {
          #
          # set grpm
          grpm <- 1
          #
        ...
        if (bool_initial || bool_distal || MinGrpAcrDst > 0) {
          ...
          #
          # get path (centers and bnd sides)
          list_pnts_path <- f_pnts_path(list_pnts_new_for_check, IDB_max, pPolygons, EndAnchorsInt, EndAnchorsDit, CfgEndStripsInt, CfgEndStripsDit, F)
          # #
          # # show step
          ...
          for (k in seq_len(nrow(pnts_split_distances))) {
            #
...
#
#
alpa_coords_length <- function(coords) {
  if (is.null(coords) || nrow(coords) < 2) {
    return(0)
  }
  sum(sqrt(diff(coords[, 1])^2 + diff(coords[, 2])^2))
}

alpa_primary_boundary_coords <- function(shp_lasld_pln, file_out = NULL) {
  candidate_lines <- list()
  for (line_group in shp_lasld_pln@lines) {
    for (line_part in line_group@Lines) {
      candidate_lines[[length(candidate_lines) + 1]] <- line_part
    }
  }
  ...
  if (length(candidate_lines) > 1) {
    message("Warning: multiple boundary lines detected, using the longest boundary line.")
    if (!is.null(file_out)) {
      alpa_write_ogr(shp_lasld_pln, ".", file_out, overwrite_layer = T, driver = "ESRI Shapefile")
    }
  }

  line_lengths <- vapply(candidate_lines, function(line_part) alpa_coords_length(line_part@coords), numeric(1))
  candidate_lines[[which.max(line_lengths)]]@coords
}
...
f_pnts_read <- function(raster_dem, shp_lasld, file_out = "lasld_pln") {
  ...
  column_IDB <- c()
  #
  # get the primary boundary line, even if holes or multipart boundaries exist
  shp_lasld_pln_coords <- alpa_primary_boundary_coords(shp_lasld_pln, file_out)
  if (is.null(shp_lasld_pln_coords)) {
    message("Error: no usable boundary line found, read pnts failed.")
    ...
    for (i in 1:(count_coords-1)) {
      ...
      if (is.na(pnt_IDB)) {
        #
        # message
        message("Warning: pnt_IDB is NA, trying relaxed boundary-point matching.")
        #
        # save
        alpa_write_ogr(shp_lasld_pln, ".", file_out, overwrite_layer = T, driver = "ESRI Shapefile")
        #
...
f_pnt_find_IDB <- function(pnt1, pnt2, pnts, raster_cellsize) {
  ...
  if (y1 == y2) {
    ...
    #
    # get IDB
    pnt_IDB <- f_pnt_find_ID(c(xmin, xmax), c(ymin, ymax), pnts, midpoint)
    if (is.na(pnt_IDB)) {
      pnt_IDB <- f_pnt_find_ID(
        c(min(x1, x2) - raster_cellsize * 0.6, max(x1, x2) + raster_cellsize * 0.6),
        c(y1 - raster_cellsize * 0.8, y1 + raster_cellsize * 0.8),
        pnts,
        midpoint,
        raster_cellsize * 1.25
      )
    }
  }
  #
  # if vertical
    ...
    if (is.na(pnt_IDB)) {
      pnt_IDB <- f_pnt_find_ID(
        c(x1 - raster_cellsize * 0.8, x1 + raster_cellsize * 0.8),
        c(min(y1, y2) - raster_cellsize * 0.6, max(y1, y2) + raster_cellsize * 0.6),
        pnts,
        midpoint,
        raster_cellsize * 1.25
      )
...
#
#
f_pnt_find_ID <- function(bndx, bndy, pnts, target = NULL, max_distance = Inf) {
  #
  # get count
  count_pnt <- nrow(pnts)
  #
  ...
  for (i in 1:count_pnt) {
    ...
    if (pntx > xmin && pntx < xmax && pnty > ymin && pnty < ymax) {
      candidate_idx <- c(candidate_idx, i)
    }
  }
  #
  ...
  #
  candidate_coords <- pnts[candidate_idx, c("Cx", "Cy"), drop = FALSE]
  candidate_dist <- sqrt((candidate_coords[, 1] - target[1])^2 + (candidate_coords[, 2] - target[2])^2)
  best_local_idx <- which.min(candidate_dist)
  if (!is.finite(candidate_dist[best_local_idx]) || candidate_dist[best_local_idx] > max_distance) {
    return(NA)
  }
  candidate_idx[best_local_idx]
}
#
#
#
...
f_pnts_clustering <- function(pnts, raster_cellsize, raster_CRS) {
  ...
  #
  # list polygons
  for (i in seq_along(pnts_polygon_sf_sparts_sp@polygons)) {
    #
    # list Polygons
    for (j in seq_along(pnts_polygon_sf_sparts_sp@polygons[[i]]@Polygons)) {
      #
      # get ID
      ...
      # print(projection(crs(pnts_polygon_sf_sparts_sp_pgn)))
      # print(is.projected(crs(pnts_shp)))
      # print(is.projected(crs(pnts_polygon_sf_sparts_sp_pgn)))
      #
      # set CRS again, or get the following
      # Error in .local(x, y, returnList, fn, ...) : 
      # 不是所有的identicalCRS(x, y)都是TRUE
      # This throws an error, because xxx and xxx
      # do not have exactly the same coordinate reference system (CRS)
      if (is.na(raster::crs(pnts_polygon_sf_sparts_sp_pgn)))
      { sp::proj4string(pnts_polygon_sf_sparts_sp_pgn) <- raster_CRS }
      #
      # get over
...
f_pnts_bnd_end_even <- function(pnts, pnts0, BoolInitial = T) {
  ...
  if (nrow(pnts_bnd) == 3) {
    ...
    for (i in 1:count_pnt_bnd) {
      ...
      if (isTRUE(!is.null(ints) && nrow(ints) == 1)) { column_unblocked <- c(column_unblocked, 1) }
...
f_angle_included <- function(agl1, agl2) {
  #
  # guard invalid angles
  if (!is.finite(agl1) || !is.finite(agl2)) {
    return(0)
  }
  #
  # get
...
f_pnts_group_end_split <- function(pnts_bnd, pnt0, end_Anchor0) {
  ...
  #
  # get path line
  lx_path <- end_Anchor0[1, "Cx"] - x0
  ly_path <- end_Anchor0[1, "Cy"] - y0
  ln_path <- sqrt(lx_path^2 + ly_path^2)
  #
  # get sorted
  pnts_bnd_ascending <- f_pnts_bnd_ascending(pnts_bnd)
  #
  ...
  #
  # get angles
  for (i in seq_len(nrow(pnts_bnd_ascending))) {
    #
    # get xy
    ...
    if (lx != 0 || ly != 0) {
      ...
      #
      # dot product
      dp <- lx * lx_path + ly * ly_path
      #
      # included angle
      agl_icld_cos <- dp / ln / ln_path
...
f_pnts_path <- function(list_pnts, IDB_max, pPolygons, EndAnchorsInt, EndAnchorsDit, EndStripsInt, EndStripsDit, BoolDirection) {
  ...
  if (count_pnts == 1) {
    ...
    #
    # get path line
    lx_path <- EndAnchorsDit[1] - EndAnchorsInt[1]
    ly_path <- EndAnchorsDit[2] - EndAnchorsInt[2]
    lz_path <- 0
    #
    # check
    for (i in seq_len(nrow(pnts_bnd_ascending))) {
      ...
      if (is.na(idx1)) { next }
      idx2 <- idx1 + 1
      if (is.na(idx2) || idx2 > nrow(pnts_bnd_ascending)) { idx2 <- 1 }
      #
      ...
      lz2 <- 0
      #
      cp1 <- pracma::cross(c(lx_path, ly_path, lz_path), c(lx1, ly1, lz1))
      cp2 <- pracma::cross(c(lx_path, ly_path, lz_path), c(lx2, ly2, lz2))
      #
      if (cp1[3] > 0 && cp2[3] < 0) {
        #
    ...
    #
    # get path line
    lx_path <- EndAnchorsInt[1] - EndAnchorsDit[1]
    ly_path <- EndAnchorsInt[2] - EndAnchorsDit[2]
    lz_path <- 0
    #
    # check
    for (i in seq_len(nrow(pnts_bnd_ascending))) {
  ...
  #
  # for debugging
  if (is.null(GrpBndAgl_initial) || is.null(GrpBndAgl_distal))
  {
  }
  # 
  # return
...
#
#
f_pnts_path_lines <- function(list_bnd_sides_right, list_bnd_sides_left, column_anchors, CfgEndStripsInitial, CfgEndStripsDistal, MinStpHrzLen) {
  # #
  # # remove Cz
  ...
  #
  # get pnts_curve
  pnts_curve_path <- column_anchors[, c("Cx", "Cy")]
  #
  # get count
  ...
  #
  # initial
  column_anchors_GrpBndInts <- column_anchors[-seq_len(nrow(column_anchors)), ]
  #
  # initial
  list_GrpBnds <- list()
  #
  ...
  if (count_grp >= 2) {
    ...
    for (i in 1:count_grp) {
      ...
      if (i != 1 && i != count_grp) {
        ...
        #
        # get intersects
        ints_upper <- f_intersection_curves(pnts_curve_path, pnts_curve_grp_bnd_upper)
        #
        # debugging
        if (is.null(ints_upper)) {
          ...
          # debugging
          f_pnts_save_points(pnts_curve_path, NA, "pnts_curve_path")
        ...
        #
        # get intersects
        ints_lower <- f_intersection_curves(pnts_curve_path, pnts_curve_grp_bnd_lower)
        #
        # debugging
  ...
  #
  # check, if end strip defined, initial
  if (length(CfgEndStripsInitial) == 5) {
    #
    # get strip end, initial
    strip_end_initial <- rbind(CfgEndStripsInitial[1:2], CfgEndStripsInitial[3:4])
    #
    # cut pnts_curve, initial
    list_cut_polyline_segment <- f_cut_polyline_segment(pnts_curve_path, strip_end_initial)
    pnts_curve_path <- list_cut_polyline_segment[[2]]
    pnts_curve_path_cut_initial <- list_cut_polyline_segment[[1]]
    #
    # get cut length
    pnts_curve_path_cut_initial_length <- sum(f_pnts_distances(pnts_curve_path_cut_initial[, c("Cx", "Cy")]))
    #
    # get strip angle end, initial
    strip_angle_end_initial <- CfgEndStripsInitial[5]
    #
    # get GrpBndAgl_ends, initial
    GrpBndAgl_end_initial <- strip_angle_end_initial
  }
  else {
    #
    # get GrpBndAgl_ends, initial
    GrpBndAgl_end_initial <- CfgEndStripsInitial
  }
  #
  # check, if end strip defined, distal
  if (length(CfgEndStripsDistal) == 5) {
    #
    # get strip end, distal
    strip_end_distal <- rbind(CfgEndStripsDistal[1:2], CfgEndStripsDistal[3:4])
    #
    # cut pnts_curve, distal
    list_cut_polyline_segment <- f_cut_polyline_segment(pnts_curve_path, strip_end_distal)
    pnts_curve_path <- list_cut_polyline_segment[[1]]
    #
    # get strip angle end, distal
    strip_angle_end_distal <- CfgEndStripsDistal[5]
    #
    # get GrpBndAgl_ends, distal
    GrpBndAgl_end_distal <- strip_angle_end_distal
  }
  else {
    #
    # get GrpBndAgl_ends, distal
    GrpBndAgl_end_distal <- CfgEndStripsDistal
  }
  #
  # get length
  length_path <- sum(f_pnts_distances(pnts_curve_path[, c("Cx", "Cy")]))
  if (!is.finite(length_path) || !is.finite(MinStpHrzLen)) {
    message("Error: invalid path length or MinStpHrzLen in f_pnts_path_lines.")
    return(NULL)
  }
  #
  # initial
  ...
  while (T) {
    #
    # update
    if (length_path/(count_strip+1) >= MinStpHrzLen) { count_strip <- count_strip+1 }
    else { break }
  }
  #
  # check, at least two strip, do not allow no station
  # even, there are end strips defined for intial and distal
  count_strip <- max(count_strip, 2)
  #
  # get strip length
  length_strip <- length_path/count_strip
  #
  # get split stations
  list_split_stations <- f_pnts_split_stations(pnts_curve_path, count_strip)
  # get
  strip_stations_length <- list_split_stations[[1]]
  # get
  ...
  if (length(CfgEndStripsInitial) == 5) {
    #
    # append
    strip_stations_length <- c(pnts_curve_path_cut_initial_length, strip_stations_length + pnts_curve_path_cut_initial_length)
    #
    # append
    strip_stations <- rbind(pnts_curve_path[1, ], strip_stations)
  }
  #
  # check, if end strip defined, distal
  if (length(CfgEndStripsDistal) == 5) {
    #
    # append
    strip_stations_length <- c(strip_stations_length, strip_stations_length[length(strip_stations_length)] + length_strip)
    #
    # append
    strip_stations <- rbind(strip_stations, pnts_curve_path[nrow(pnts_curve_path), ])
  }
  #
  # initial
  ...
  for (i in 1:count_station) {
    ...
    strip_station_Lhrz <- strip_stations_length[i]
    #
    # check, if end strip defined, initial
    if (i == 1 && length(CfgEndStripsInitial) == 5) {
      #
      # get strip angle
      strip_angle <- strip_angle_end_initial
    }
    else if (i == count_station && length(CfgEndStripsDistal) == 5) {
      #
      # get strip angle
      strip_angle <- strip_angle_end_distal
    }
    else {
      #
      # check, if, no group boundary, possibly only one group
      if (count_grp_bnd >= 2) {
        ...
        if (length(CfgEndStripsInitial) == 5) {
          #
          # check, if within the initial group
          if (index_GrpBndInt_upper == 0) {
            #
            # update
            angle_grp_bnd_upper <- strip_angle_end_initial
            Lhrz_diff <- column_GrpBndInts[index_GrpBndInt_lower, "Lhrz"] - strip_stations_length[1]
            Lhrz_diff_station <- strip_station_Lhrz - strip_stations_length[1]
          }
          #
          # check, if index_GrpBndInt_upper is upper than end strip, initial
          else if (column_GrpBndInts[index_GrpBndInt_upper, "Lhrz"] <= strip_stations_length[1]) {
            #
            # update
            angle_grp_bnd_upper <- strip_angle_end_initial
            Lhrz_diff <- column_GrpBndInts[index_GrpBndInt_lower, "Lhrz"] - strip_stations_length[1]
        ...
        if (length(CfgEndStripsDistal) == 5) {
          #
          # check, if within the distal group
          if (index_GrpBndInt_lower == count_grp_bnd+1) {
            #
            # update
            angle_grp_bnd_lower <- strip_angle_end_distal
            Lhrz_diff <- strip_stations_length[length(strip_stations_length)] - column_GrpBndInts[index_GrpBndInt_upper, "Lhrz"]
            Lhrz_diff_station <- strip_station_Lhrz - column_GrpBndInts[index_GrpBndInt_upper, "Lhrz"]
          }
          #
          # check, if index_GrpBndInt_lower is lower than end strip, distal
          else if (column_GrpBndInts[index_GrpBndInt_lower, "Lhrz"] >= strip_stations_length[length(strip_stations_length)]) {
            #
            # update
            angle_grp_bnd_lower <- strip_angle_end_distal
            Lhrz_diff <- strip_stations_length[length(strip_stations_length)] - column_GrpBndInts[index_GrpBndInt_upper, "Lhrz"]
        ...
        if (strip_station_Lhrz <= column_anchors[2, "Lhrz"]) {
          ...
          if (length(CfgEndStripsInitial) == 5) {
            ...
            # update
            angle_grp_bnd_upper <- strip_angle_end_initial
            Lhrz_diff <- column_anchors[2, "Lhrz"] - strip_stations_length[1]
            Lhrz_diff_station <- strip_station_Lhrz - strip_stations_length[1]
          }
        }
        else {
          ...
          if (length(CfgEndStripsDistal) == 5) {
            ...
            # update
            angle_grp_bnd_lower <- strip_angle_end_distal
            Lhrz_diff <- strip_stations_length[length(strip_stations_length)] - column_anchors[2, "Lhrz"]
            Lhrz_diff_station <- strip_station_Lhrz - column_anchors[2, "Lhrz"]
          }
        }
      }
    ...
    #
    # debugging
    if (FALSE && length(strip_angle) == 0) {
      #
      # message
      message("Error: strip_angle is of length zero.")
    }
    #
    # update
...
f_pnts_split_station <- function(pnts, length_cumus, length_split) {
  ...
  #
  # get 
  for (i in seq_along(length_cumus)) {
    #
...
f_pnts_split_stations <- function(pnts, count_split = 1) {
  ...
  #
  # return
  return(list(stationing[2:count_split], column_split_stations))
}
#
#
#
f_strips <- function(pPolygons, list_path, CfgEndStripsInitial, CfgEndStripsDistal, MinStpHrzLen) {
  #
  # get anchors
  ...
  #
  # get path lines
  list_path_lines <- f_pnts_path_lines(list_bnd_sides_right, list_bnd_sides_left, column_anchors, CfgEndStripsInitial, CfgEndStripsDistal, MinStpHrzLen)
  #
  # debugging
  if (is.null(list_path_lines)) {
    #
    # save
    f_pnts_save_points(column_anchors, NA, "column_anchors")
    #
  ...
  if (is.null(spldf_cls)) {
    #
    # sometimes, path will go through outside landslide polygon?
    #
    # message
  ...
  #
  # check, station might outside landslide polygon???
  for (i in 1:count_cl) {
    #
    # get coords
    ...
    # This throws an error, because xxx and xxx
    # do not have exactly the same coordinate reference system (CRS)
    sp::proj4string(sshp) <- raster::crs(pSpatialPolygons)
    #
  ...
  bool_ints <- TRUE
  #
  # check clipping lines, delete intersected cutlines
  while (bool_ints && count_cl >= 2) {
    ...
    for (i in 1:(count_cl-1)) {
      ...
      #
      # get coords
      sfc_cl_i_coords <- matrix(sfc_cl_i, nc = 2, byrow = FALSE)
      #
      # check
      for (k in (i+1):count_cl) {
        ...
        #
        # get coords
        sfc_cl_k_coords <- matrix(sfc_cl_k, nc = 2, byrow = FALSE)
        #
        # get ints
    ...
    #
    # if end strip defined, do not delete end strip cutline
    if (length(CfgEndStripsInitial) == 5) { column_ints_count[1] <- 0 }
    if (length(CfgEndStripsDistal) == 5) { column_ints_count[count_cl] <- 0 }
    #
    # check
  ...
  # when consider station as the intersection of path and strip
  ...
  for (i in 1:count_cl) {
    ...
    sfc_st_split_POLYGONs <- sf::st_collection_extract(sfc_st_split, type = c("POLYGON"))
    #
    # get count
    ...
    if (count_POLYGON == 1 || count_POLYGON >= 3) {
      #
      # message
      message("Error: Polygon cutting return 1 or more than 3 sub-polygons.")
      message("Possibly, the extending of EndStrips is still in landslide polygon.")
      message("The first suggestion is to modify EndStrips to fix this error.")
      #
      # save
    ...
    # This throws an error, because xxx and xxx
    # do not have exactly the same coordinate reference system (CRS)
    sp::proj4string(pnt_upper_fwd_shp) <- raster::crs(pSpatialPolygons)
    sp::proj4string(sfc_POLYGON1_sp) <- raster::crs(pSpatialPolygons)
    sp::proj4string(sfc_POLYGON2_sp) <- raster::crs(pSpatialPolygons)
    #
    # get over
    ...
    if (is.null(int.coords)) {
      ...
      #
      # save
      f_pnts_save_points(data.frame(int.coords), NA, "int.coords")
      #
      # message
      message("Error: path and strip do not have intersections.")
      #
      # return
      return(NULL)
    }
    ...
    if (nrow(int.coords) >= 3) {
      ...
      #
      # message
      message("Error: path and strip have more than 2 intersections.")
      # #
      # # keep the first and last intersections?
      # int.coords <- int.coords[c(1, nrow(int.coords)), ]
      #
      # NOTE: do not need to out files for debugging
      # This NULL will let f_strips_package return empty list()
      # and, let idx_hierarchical <- idx_hierarchical - 1
      # i.e., search the next valid hierarchical sub-grouping of elevation points
      #
      # return
      return(NULL)
    }
  ...
  if (T) {
    ...
    while (T) {
      ...
      if (is.null(int.coords) || nrow(int.coords) >= 3) {
        ...
        #
        # message
        message("Error: path and strip do not have 2 intersections.")
        #
        # This happens for Kyrgyz_020, 
        # when landslide path intersects with the distal end strip boundary twice.
        # This will trigger the merging of end sub- groups to form a new end group.
        #
        # return
        return(NULL)
      }
...
f_strips_clipping_polylines <- function(pPolygons, strip_stations, strip_stations_angles, CRS_out = NA) {
  ...
  #
  # get Lines
  for (i in seq_len(nrow(strip_stations))) {
    #
    # get station
  ...
  #
  # set names
  rownames(data_df) <- seq_len(nrow(data_df))
  #
  # get spldf
...
#
#
f_strips_areas <- function(pRasterDEM, list_strip_sp, strip_PATH_ends, strip_nodes) {
  # #
  # # save, for debugging
  # f_pnts_save_points(strip_nodes, NA, "strip_nodes")
  #
  ...
  for (i in seq_len(nrow(column_triangles))) {
    # #
    # # get triangle, for debugging
    # triangle <- data.frame(rbind(column_coords[column_triangles[i, 1], c("Cx", "Cy")], 
    #                              column_coords[column_triangles[i, 2], c("Cx", "Cy")], 
    #                              column_coords[column_triangles[i, 3], c("Cx", "Cy")]))
    #
    # get ints, edge1
    ...
    # if (length(int.coords) == 1 && int.coords == "invalid") { f_pnts_save_points(triangle, NA, "triangle") }
    #
  ...
  #
  # get positions
  for (i in seq_len(nrow(column_ints_rmdp))) {
    #
    # get
  ...
  #
  # sort, for the second time
  for (i in seq_along(column_positions_unique)) {
    #
    # get index
  ...
  for (i in 1:count_node) {
    ...
    if (strip_nodes2[i, "type"] != "Anchor: Initial group anchor" &&
      ...
      #
      # debugging
      if(is.na(index_anchor_upper) || is.na(index_anchor_lower)) {
        #
        # message
        message("Error: index_anchor is NA.")
        #
        # pause at here
        return(NULL)
      }
  ...
  for (i in 1:count_node) {
    ...
    # to the intersection with path
    # and, do not extend only a little amount
    # as you already get intersection with path
    ...
    #
    # get Czs
    for (k in seq_len(nrow(column_triangles))) {
      #
      # get index
  ...
  #
  # if end strip is PATH strip
  strip_PATHs <- data.frame(numeric(count_strip)) + 1
  strip_PATHs[1, 1] <- strip_PATH_ends[1]
  strip_PATHs[nrow(strip_PATHs), 1] <- strip_PATH_ends[2]
  #
  # get sp attribute
  strip_df <- cbind(column_Lhrzs, column_Lalls, column_Ahrzs, column_Aalls, column_Whrzs, column_Walls, column_drops, strip_PATHs)
  #
  # colnames
  colnames(strip_df) <- c("Lhrz", "Lall", "Ahrz", "Aall", "Whrz", "Wall", "Drop", "PATH")
  #
  # get
...
f_strips_package <- function(list_pnts, IDB_max, pPolygons, EndAnchorsInt, EndAnchorsDit, EndStripsInt, EndStripsDit, MinStpHrzLen, pRasterDEM) {
  ...
  #
  # get path (centers and bnd sides)
  list_pnts_path <- f_pnts_path(list_pnts, IDB_max, pPolygons, EndAnchorsInt, EndAnchorsDit, CfgEndStripsInt, CfgEndStripsDit, T)
  #
  # check
  if (is.null(list_pnts_path)) {
    #
  ...
  pnts_split_anchors <- list_pnts_path[[1]]
  #
  # get end strip angles
  strip_end_agls <- list_pnts_path[[4]]
  #
  # must do this replace???
  # something is done in f_pnts_path???
  if (length(EndStripsInt) == 5) { EndStripsInt[5] <- strip_end_agls[1] }
  if (length(EndStripsDit) == 5) { EndStripsDit[5] <- strip_end_agls[2] }
  #
  # check, if end strip defined, initial
  if (length(EndStripsInt) == 5) {
    #
    # get strip end, initial
    strip_end_initial <- rbind(EndStripsInt[1:2], EndStripsInt[3:4])
    #
    # get split
    split_polygon_segment <- f_split_polygon_segment(pPolygons, strip_end_initial, 1/1000)
    split_polygon_segment_areas <- sf::st_area(split_polygon_segment)
    pPolygons_areas <- sf::st_area(sf::st_as_sf(sp::SpatialPolygons(list(pPolygons))))
    # get cut
    list_cut_polyline_segment <- f_cut_polyline_segment(pnts_split_anchors[, c("Cx", "Cy")], strip_end_initial)
    #
    # check, satify both
    # 1. 将landslide polygon切割为2个sub-polygon
    # 2. 将path切割为多于2个sub-polyline???
    if (!is.null(split_polygon_segment) && nrow(split_polygon_segment) == 2 && min(split_polygon_segment_areas) > sum(pPolygons_areas) / 10^6 &&
        !is.null(list_cut_polyline_segment) && nrow(list_cut_polyline_segment[[1]]) >= 2) { 
      #
      CfgEndStripsInt <- EndStripsInt
    }
    else { 
      #
      CfgEndStripsInt <- EndStripsInt[5]
    }
  }
  else {
    #
    CfgEndStripsInt <- strip_end_agls[1]
  }
  #
  # check, if end strip defined, distal
  if (length(EndStripsDit) == 5) {
    #
    # get strip end, distal
    strip_end_distal <- rbind(EndStripsDit[1:2], EndStripsDit[3:4])
    #
    # get split
    split_polygon_segment <- f_split_polygon_segment(pPolygons, strip_end_distal, 1/1000)
    split_polygon_segment_areas <- sf::st_area(split_polygon_segment)
    pPolygons_areas <- sf::st_area(sf::st_as_sf(sp::SpatialPolygons(list(pPolygons))))
    # get cut
    list_cut_polyline_segment <- f_cut_polyline_segment(pnts_split_anchors[, c("Cx", "Cy")], strip_end_distal)
    #
    # check, satify both
    # 1. 将landslide polygon切割为2个sub-polygon
    # 2. 将path切割为多于2个sub-polyline???
    if (!is.null(split_polygon_segment) && nrow(split_polygon_segment) == 2 && min(split_polygon_segment_areas) > sum(pPolygons_areas) / 10^6 &&
        !is.null(list_cut_polyline_segment) && nrow(list_cut_polyline_segment[[2]]) >= 2) { 
      #
      CfgEndStripsDit <- EndStripsDit
    }
    else { 
      #
      CfgEndStripsDit <- EndStripsDit[5]
    }
  }
  else { 
    #
    CfgEndStripsDit <- strip_end_agls[2]
  }
  #
  # get strips, strip might be merged
  list_strips <- f_strips(pPolygons, list_pnts_path, CfgEndStripsInt, CfgEndStripsDit, MinStpHrzLen)
  #
  # check
  if (is.null(list_strips)) {
    ...
    #
    # if end strip is not PATH strip
    strip_PATH_ends <- c(ifelse(length(CfgEndStripsInt) == 5, 0, 
                                ifelse(length(CfgEndStripsInt) == 1, 1, NA)),
                         ifelse(length(CfgEndStripsDit) == 5, 0, 
                                ifelse(length(CfgEndStripsDit) == 1, 1, NA)))
    #
    # get areas
    list_areas <- f_strips_areas(pRasterDEM, list_strip_sp, strip_PATH_ends, strip_nodes_for_sp)
    #
    # get 
...
f_intersection_curves <- function(pnts1, pnts2) {
  #
  # ensure type correct, from data.frame to numeric matrix
  pnts1 <- as.matrix(pnts1)
  pnts2 <- as.matrix(pnts2)
  #
  # build sf lines
  line1_sf <- sf::st_sf(
    id = 1,
    geometry = sf::st_sfc(sf::st_linestring(pnts1))
  )
  line2_sf <- sf::st_sf(
    id = 2,
    geometry = sf::st_sfc(sf::st_linestring(pnts2))
  )
  #
  # calculate locations where curves intersect
  int_geom <- suppressWarnings(sf::st_intersection(sf::st_geometry(line1_sf), sf::st_geometry(line2_sf)))
  if (length(int_geom) == 0) {
    return(NULL)
  }
  #
  # get coords from intersection geometry
  int.coords <- c()
  int_geom_types <- as.character(sf::st_geometry_type(int_geom, by_geometry = TRUE))
  non_collection_geom <- int_geom[int_geom_types != "GEOMETRYCOLLECTION"]
  if (length(non_collection_geom) > 0) {
    direct_coords <- suppressWarnings(sf::st_coordinates(non_collection_geom))
    if (!is.null(direct_coords) && nrow(direct_coords) > 0) {
      int.coords <- rbind(int.coords, direct_coords[, c("X", "Y"), drop = FALSE])
    }
  }
  collection_geom <- int_geom[int_geom_types == "GEOMETRYCOLLECTION"]
  if (length(collection_geom) > 0) {
    point_geom <- suppressWarnings(sf::st_collection_extract(collection_geom, "POINT"))
    if (length(point_geom) > 0) {
      point_coords <- suppressWarnings(sf::st_coordinates(point_geom))
      if (!is.null(point_coords) && nrow(point_coords) > 0) {
        int.coords <- rbind(int.coords, point_coords[, c("X", "Y"), drop = FALSE])
      }
    }
    line_geom <- suppressWarnings(sf::st_collection_extract(collection_geom, "LINESTRING"))
    if (length(line_geom) > 0) {
      line_coords <- suppressWarnings(sf::st_coordinates(line_geom))
      if (!is.null(line_coords) && nrow(line_coords) > 0) {
        int.coords <- rbind(int.coords, line_coords[, c("X", "Y"), drop = FALSE])
      }
    }
  }
  if (is.null(int.coords) || length(int.coords) == 0 || nrow(as.matrix(int.coords)) == 0) {
    return(NULL)
  }
  int.coords <- unique(as.matrix(int.coords))
  colnames(int.coords) <- c("Cx", "Cy")
  #
  # check
  # possibly get duplicate intersections???
  # may be because ployline has back-forward segment???
  # for example, the point series p1, p2, p3, p2, p4, p5
  # then, p2-p3, p3-p2, are two identical segments
  # do not remove duplicate intersection
  #
  # return
  return(int.coords)
}
...
#
#
f_cut_polyline_segment <- function(polyline, segment) {
  #
  # ensure type correct, from data.frame to numeric
  polyline <- as.matrix(polyline)
  #
  # sf
  polyline_sf <- sf::st_sf(
    id = 1, 
    geometry = sf::st_sfc(sf::st_linestring(polyline))
  )
  # sf
  segment_sf <- sf::st_sf(
    id = 2, 
    geometry = sf::st_sfc(sf::st_linestring(segment))
  )
  # #
  # # set crs
  # st_crs(polyline_sf) <- 4326
  # st_crs(segment_sf) <- 4326
  #
  # st_intersection, return POINT or MULTIPOINT
  intersection_pts <- sf::st_intersection(polyline_sf, segment_sf)
  #
  # if no intersections
  if (nrow(intersection_pts) == 0) {
    # #
    # # message
    # message("Error: no interactions.")
    #
    # return
    return(NULL)
  }
  #
  # if more than 1 intersections
  if (nrow(intersection_pts) >= 2) {
    # #
    # # message
    # message("Error: more than 1 interactions.")
    #
    # return
    return(NULL)
  }
  #
  # get count
  count_node = nrow(polyline)
  #
  for (i in 1:(count_node-1)) {
    #
    # sf
    polyline_segment_sf <- sf::st_sf(
      id = 3, 
      geometry = sf::st_sfc(sf::st_linestring(polyline[i:(i+1), ]))
    )
    #
    # st_intersection
    intersection_pts <- sf::st_intersection(polyline_segment_sf, segment_sf)
    #
    # check
    if (nrow(intersection_pts) == 1) {
      #
      # get coords
      intersections_coords <- sf::st_coordinates(intersection_pts)
      # get XY
      intersections_coords <- unique(intersections_coords[, c("X", "Y")])
      #
      # get segments
      polyline_segment_upper <- unique(rbind(polyline[1:i, ], intersections_coords))
      polyline_segment_lower <- unique(rbind(intersections_coords, polyline[(i+1):count_node, ]))
      #
      # if intersect with polyline ends, possibly return one coordinates
      return(list(polyline_segment_upper, polyline_segment_lower))
    }
  }
}
#
#
#
f_split_polygon_segment <- function(pPolygons, segment, RatioEXTD = NA) {
  #
  # Gemini:
  # pPolygons 是单个 "Polygons" 对象，st_as_sf 无法直接识别。
  # 必须先用 list() 包裹，再通过 sp::SpatialPolygons() 提升为完整的空间对象。
  pSpatialPolygons <- sp::SpatialPolygons(list(pPolygons))
  #
  # 将 S4 (sp) Polygon 转换为 sf 对象
  # 您的 ALPA 代码中经常混用 sp 和 sf，这里转为 sf 处理更方便
  sf_poly <- sf::st_as_sf(pSpatialPolygons)
  #
  # check, if extending the segment by RatioEXTD
  if (is.numeric(RatioEXTD) && RatioEXTD > 0 && is.finite(RatioEXTD)) {
    #
    # get diff
    pt1 <- segment[1, ]
    pt2 <- segment[2, ]
    ...
    #
    # get coords
    pt1_extd_x <- pt1[1] - dx * RatioEXTD
    pt1_extd_y <- pt1[2] - dy * RatioEXTD
    pt2_extd_x <- pt2[1] + dx * RatioEXTD
    pt2_extd_y <- pt2[2] + dy * RatioEXTD
    #
    # 构建延长后的新线段矩阵
    segment <- matrix(
      c(pt1_extd_x, pt1_extd_y, pt2_extd_x, pt2_extd_y), 
      nrow = 2, byrow = TRUE)
  }
  #
  # 根据两个二维坐标点构建线段 (LineString)
  sf_line <- sf::st_linestring(segment)
  #
  # 将线段转换为 sfc 对象，并确保坐标系与 Polygon 一致
  sf_sfc_line <- sf::st_sfc(sf_line, crs = sf::st_crs(sf_poly))
  #
  # 执行 Split 操作
  # 使用 lwgeom::st_split 进行切割
  # 注意：如果线段没有完全贯穿多边形（即没有接触到两边边界），
  # st_split 可能不会将其分为两个独立的多边形，除非线段构成了闭合环或连接了边界。
  split_collection <- lwgeom::st_split(sf_poly, sf_sfc_line)
  #
  # 提取切割结果中的 Polygon
  # st_split 的结果通常是 GEOMETRYCOLLECTION，需要提取出 POLYGON 类型
  split_polys <- sf::st_collection_extract(split_collection, type = "POLYGON")
  #
  # 返回结果（数量和切割后的 sf 对象）
  return(split_polys)
}
...
f_pnts_save_points <- function(pnts, CRS_out, file_out) {
  ...
  if (is.null(PntsCoords_sptdf)) {
    return(NULL)
  }
  #
  # write
  alpa_write_ogr(PntsCoords_sptdf, dsn = ".", layer = file_out, overwrite = T, driver="ESRI Shapefile")
  # #
...
#
#
f_pnts_save_path <- function(pnts_path, paras_strip, iName, MinGrpPntCnt, MinGrpAcrDst, MinStpHrzLen, MinEndRtoInt, MinEndRtoDit, EndAnchorsInt, EndAnchorsDit, EndStripsInt, EndStripsDit, CRS_out, file_out, LineID = 0) {
  #
  # get parameters for landslide path
  ...
  # I think, even if extreme coincidence happen 
  ...
  #
  # check
  if (length(EndStripsInt) == 1 && EndStripsInt < 0) { EndStripsInt <- abs(EndStripsInt) }
  else { EndStripsInt[5] <- EndStripsInt[5] * 180 / pi }
  if (length(EndStripsDit) == 1 && EndStripsDit < 0) { EndStripsDit <- abs(EndStripsDit) }
  else { EndStripsDit[5] <- EndStripsDit[5] * 180 / pi }
  #
  # get data.frame
  if (length(EndAnchorsInt) == 2 && length(EndAnchorsDit) == 2) {
    #
    if (length(EndStripsInt) == 5 && length(EndStripsDit) == 5) {
      #
      paras_input <- data.frame(iMGPC = MinGrpPntCnt, 
                                ...
                                iESI1x = EndStripsInt[1], 
                                iESI1y = EndStripsInt[2], 
                                iESI2x = EndStripsInt[3], 
                                iESI2y = EndStripsInt[4], 
                                iESIdir = EndStripsInt[5], 
                                iESD1x = EndStripsDit[1], 
                                iESD1y = EndStripsDit[2], 
                                iESD2x = EndStripsDit[3], 
                                iESD2y = EndStripsDit[4], 
                                iESDdir = EndStripsDit[5])
    }
    #
    if (length(EndStripsInt) == 5 && length(EndStripsDit) == 1) {
      #
      paras_input <- data.frame(iMGPC = MinGrpPntCnt, 
                                iMGAD = MinGrpAcrDst, 
                                iMSHL = MinStpHrzLen, 
    ...
    #
    if (length(EndStripsInt) == 1 && length(EndStripsDit) == 5) {
      #
      paras_input <- data.frame(iMGPC = MinGrpPntCnt, 
                                iMGAD = MinGrpAcrDst, 
                                iMSHL = MinStpHrzLen, 
    ...
    #
    if (length(EndStripsInt) == 1 && length(EndStripsDit) == 1) {
      #
      paras_input <- data.frame(iMGPC = MinGrpPntCnt, 
                                iMGAD = MinGrpAcrDst, 
                                iMSHL = MinStpHrzLen, 
  ...
  #
  # get spldf
  spldf <- f_pnts2shp_polyline(pnts_path, CRS_out, data_df)
  #
  # write
  alpa_write_ogr(spldf, dsn = ".", layer = file_out, overwrite = T, driver="ESRI Shapefile")
  #
  # return
  return(spldf)
}
...
f_pnts2shp_points <- function(pnts, CRS_out) {
  ...
  # get coords
  PntsCoords <- pnts
  valid_rows <- is.finite(PntsCoords[, "Cx"]) & is.finite(PntsCoords[, "Cy"])
  PntsCoords <- PntsCoords[valid_rows, , drop = FALSE]
  if (nrow(PntsCoords) == 0) {
    return(NULL)
  }
...
#
#
f_pnts2paras_path <- function(pnts, paras_strip) {
  #
  # get count
  count_pnts = nrow(pnts)
  #
  # initial, displacement for the whole path
  l_path = 0
  h_path = 0
  s_path = 0
  #
  ...
  for (i in 2:count_pnts) {
    #
    # get residual height
    g_series <- rbind(g_series, (-h_series[i]) - miuapp_path * l_series[i])
  }
  #
  # initial
  H_integral <- 0
  ...
  T_overall <- s_path / d_overall
  #
  # get tortuosity horizontal
  d_horizontal <- sqrt((pnts[count_pnts, "Cx"] - pnts[1, "Cx"])^2 + 
                         (pnts[count_pnts, "Cy"] - pnts[1, "Cy"])^2)
  T_horizontal <- l_path / d_horizontal
  #
  # get tortuosity longitudinal
  d_longitudinal <- sqrt((l_path)^2 +
                           (h_path)^2)
  T_longitudinal <- s_path / d_longitudinal
  #
  # initial
  paras <- c()
  #
  ...
  if (is.null(paras_strip)) {
    #
    # width no
    paras <- data.frame(Dall = d_overall, Lall = s_path,
                        Dhrz = d_horizontal, Lhrz = l_path,
                        Dlng = d_longitudinal,
                        Hfnl = -h_path,
...
f_save_list2sp <- function(list_sp, data_df, CRS_out, file_out) {
  ...
  #
  # write
  alpa_write_ogr(spdf, dsn = ".", layer = file_out, overwrite = T, driver="ESRI Shapefile")
}
#
#
#
...
#
#
f_save_paths_png <- function(pRasterDEM, shp_lasld, list_spldf, file_out) {
  #
  # return if no path extracted yet
  if (length(list_spldf) == 0) {
    return(invisible(FALSE))
  }
  shp_lasld_sf <- sf::st_as_sf(shp_lasld)
  list_spldf_sf <- lapply(list_spldf, sf::st_as_sf)
  #
  # get extent and add a small margin so the overlay is readable
  bbox_vals <- sf::st_bbox(shp_lasld_sf)
  x_margin <- max((bbox_vals["xmax"] - bbox_vals["xmin"]) * 0.05, .Machine$double.eps)
  y_margin <- max((bbox_vals["ymax"] - bbox_vals["ymin"]) * 0.05, .Machine$double.eps)
  #
  # save a simple overlay of landslide boundary and final profile path
  grDevices::png(filename = file_out, width = 2400, height = 1800, res = 200)
  on.exit(grDevices::dev.off(), add = TRUE)
  graphics::par(mar = c(3.5, 3.5, 2.5, 2.5), bg = "white")
  plot(
    sf::st_geometry(shp_lasld_sf),
    xlim = c(bbox_vals["xmin"] - x_margin, bbox_vals["xmax"] + x_margin),
    ylim = c(bbox_vals["ymin"] - y_margin, bbox_vals["ymax"] + y_margin),
    axes = TRUE,
    border = "#264653",
    col = NA,
    lwd = 1.4,
    main = "Landslide boundary and profile path"
  )
  for (spldf_i in list_spldf_sf) {
    plot(sf::st_geometry(spldf_i), add = TRUE, col = "#D62828", lwd = 2.5)
  }
  invisible(file_out)
}
#
